...
trait ClientLogic {
  ...
  def handleJoinCommand (data: Map[String, Any]) : Unit =
    (data get "server", data get "channel") match {
  ...
  def handlePrivmsgCommand (data: Map[String, Any]) : Unit =
    (data get "server", data get "receiver", data get "text") match {
